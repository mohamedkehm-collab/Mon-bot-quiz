...
function formatScore(chatId) {
  ...
  if (entries.length === 0) {
    return { texte: 'Aucun score enregistré pour le moment.', mentions: [] };
  }
  let texte = '🏆 Classement\n\n';
  entries.forEach(([, data], i) => {
    texte += `${i + 1}. ${data.nom} — ${data.points} pt(s)\n`;
  });
  return { texte, mentions: [] };
}
...
async function startBot() {
  ...
  // ---- Envoie un message en gras, avec mentions optionnelles ----
  async function envoyer(chatId, texte, mentions) {
    const payload = { text: `*${texte}*` };
    if (mentions && mentions.length > 0) payload.mentions = mentions;
    await sock.sendMessage(chatId, payload);
  }

  // ---- Envoie le classement (déjà en gras + mentions intégrées) ----
  async function envoyerScore(chatId) {
    const { texte, mentions } = formatScore(chatId);
    await envoyer(chatId, texte, mentions);
  }
  ...
  async function poserQuestion(chatId) {
    ...
    if (session.total !== null && session.count >= session.total) {
      ...
      await envoyer(chatId, `🏁 Quiz terminé ! ${session.total} question(s) posée(s).`);
      await envoyerScore(chatId);
      return;
    }
    ...
    if (!q) {
      await envoyer(chatId, '⚠️ Aucune question disponible. Vérifie le fichier questions.xlsx.');
      session.running = false;
      await envoyerScore(chatId);
      return;
    }
    ...
    await envoyer(chatId, formatQuestion(q, session.count, session.total));

    const timeoutId = setTimeout(async () => {
      const quiz = activeQuizzes[chatId];
      if (quiz && !quiz.resolved) {
        quiz.resolved = true;
        await envoyer(chatId, `⏰ Temps écoulé ! La bonne réponse était : ${quiz.question.bonneReponse}`);
    ...
    try {
      ...
      if (bodyLower === '!aide' || bodyLower === '!help') {
        await envoyer(
          ...
          '🤖 Commandes disponibles\n\n' +
            '!quiz → démarrer une série de 25 questions, avec classement final automatique (réservé à l’organisateur)\n' +
            '!quiz30 → démarrer une série de 30 questions précises (ou tout autre nombre) (réservé à l’organisateur)\n' +
            '!stop → arrêter la série de questions et afficher le score final (réservé à l’organisateur)\n' +
      ...
      if (matchQuiz) {
        if (!estProprietaire(senderId)) {
          await envoyer(chatId, '🔒 Seul l’organisateur peut démarrer le quiz.');
          return;
        }
        if (sessions[chatId]?.running) {
          await envoyer(chatId, 'Un quiz est déjà en cours ! Tape !stop pour l’arrêter.');
          return;
        }
        const total = matchQuiz[1] ? parseInt(matchQuiz[1], 10) : 25;
        if (total !== null && total <= 0) {
          await envoyer(chatId, 'Indique un nombre de questions valide, ex : !quiz30');
          return;
        }
        sessions[chatId] = {
          running: true,
      ...
      if (bodyLower === '!stop') {
        if (!estProprietaire(senderId)) {
          await envoyer(chatId, '🔒 Seul l’organisateur peut arrêter le quiz.');
          return;
        }
        if (sessions[chatId]) {
        ...
        delete activeQuizzes[chatId];
        await envoyer(chatId, '🛑 Quiz arrêté.');
        await envoyerScore(chatId);
        return;
      }
      ...
      if (bodyLower === '!sticker' && isReplyToImage) {
        if (!estProprietaire(senderId)) {
          await envoyer(chatId, '🔒 Seul l’organisateur peut créer des stickers.');
          return;
        }
        await envoyer(chatId, '🎨 Création du sticker...');
      ...
      if (quiz && !quiz.resolved && !msg.key.fromMe && textMessage.trim() !== '' && !estUneCommande) {
        ...
        if (estCorrecte) {
          ...
          await envoyer(chatId, `✅ Bonne réponse, ${senderName} ! (+10 points)`);
          delete activeQuizzes[chatId];
          ...
          await envoyer(chatId, `❌ Mauvaise réponse, ${senderName}. Essaie encore !`);
        }
      }
    } catch (err) {
