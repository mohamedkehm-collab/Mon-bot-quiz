...
function estProprietaire(senderId) {
  ...
  return OWNER_NUMBERS.includes(numero);
}
...
async function startBot() {
  ...
  async function poserQuestion(chatId) {
    ...
    session.count += 1;
    activeQuizzes[chatId] = { question: q, dejaRepondu: new Set(), resolved: false };
    ...
    try {
      ...
      if (matchQuiz) {
        if (!estProprietaire(senderId)) {
          await sock.sendMessage(chatId, { text: '🔒 Seul l’organisateur peut démarrer le quiz.' });
          return;
        }
        if (sessions[chatId]?.running) {
      ...
      if (bodyLower === '!stop') {
        if (!estProprietaire(senderId)) {
          await sock.sendMessage(chatId, { text: '🔒 Seul l’organisateur peut arrêter le quiz.' });
          return;
        }
        if (sessions[chatId]) {
      ...
      if (quiz && textMessage.trim() !== '' && !estUneCommande) {
        if (quiz.dejaRepondu.has(senderId)) return; // déjà répondu à cette question

        quiz.dejaRepondu.add(senderId);
